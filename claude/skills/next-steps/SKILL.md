...
- What is the spec/plan's `status:`? (`not-started` / `in-progress` / `blocked` / `agent-done` / `done` / `abandoned`; older files may say `draft` / `active` / `complete` / `superseded`). `agent-done` means the agent finished but the human has not accepted it — report it as awaiting their check, not as finished
