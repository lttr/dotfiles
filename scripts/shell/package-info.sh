...
# zsh doesn't support type -t, so use whence -w instead
COMMAND_TYPE=$(whence -w "$COMMAND" 2>/dev/null | awk '{print $2}')

case "$COMMAND_TYPE" in
    "alias")
        ALIAS_DEF=$(alias "$COMMAND" 2>/dev/null | sed "s/^$COMMAND='//; s/'$//" | sed "s/^$COMMAND=//")
